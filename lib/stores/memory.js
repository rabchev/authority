...
function reversIndex(man, acl) {

    function setPermision(perm, opKey) {
        var roles;
        if (perm) {
    ...
    if (acl.operation) {
        ...
        ops.forEach(function (opKey) {
            ...
            setPermision(op.deny, opKey);
            setPermision(op.allow, opKey);
        });
    }
}
...
MemoryStore.prototype.getPermissions = function (pattern, done) {
    if (!done) {
        done = pattern;
        pattern = null;
    }
